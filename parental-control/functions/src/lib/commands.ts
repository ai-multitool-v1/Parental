 ...
 * device do. Every entry mirrors a handler in the Android CommandProcessor;
 * anything not listed here is refused before a doc is ever created.
 *
 * Clients cannot write devices/{id}/commands directly (firestore.rules:
 * create=false for everyone) — this module is the only writer, which makes
 * every command authenticated, validated, TTL-bound and audited upstream.
 */
 ...
 * are capped, and types are coerced to safe primitives. The website may send
 * a plain string payload for SEND_NOTIFICATION (message body shorthand).
 */
...
// PREMIUM_COMMAND_TYPES is re-exported for dispatchCommand's plan gate so
// the premium list lives beside the whitelist it indexes into.
